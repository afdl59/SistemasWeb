const physics = {
  ...
  sceneryCollisionCheck(data, entities, scenery) {
    entities.forEach((entity) => {
      scenery.forEach((scene) => {
        // Skip decorative elements (no collision)
        if (scene.type === 'mountain' || scene.type === 'smallCloud' || 
            scene.type === 'mediumCloud' || scene.type === 'largeCloud' || 
            scene.type === 'shrub' || scene.type === 'flag' || 
            scene.type === 'flagpole' || scene.type === 'castle') {
          return;
        }
  ...
  sceneryCollision(data, entity, scene) {
    // Left side collision
    if (entity.xPos < scene.xPos && 
        entity.xPos + entity.width > scene.xPos &&
        entity.yPos + entity.height > scene.yPos + 2 &&
        entity.yPos < scene.yPos + scene.height - 2) {
    ...
    // Top (landing on ground/platforms)
    ...
    if (entity.yPos >= scene.yPos &&
      ...
      if (scene.type === 'block') {
        ...
        if (entity.bigMario) {
          if (data.sounds && data.sounds.breakSound) {
